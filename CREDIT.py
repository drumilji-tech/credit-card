...
def main():
    ...
    if Model == "Logistic Regression":
        ...
        if Model == "Decision Tree":
            st.sidebar.subheader("Model Hyperparameters")
            max_depth_tree = st.sidebar.number_input("The maximum depth of the tree",1,20,step=1,key='max_depth')
            criteria_tree = st.sidebar.radio("Criterion For Decision Tree",('Gini','Entropy'),key='criteria')
            metrics_tree = st.sidebar.selectbox("Which metrics to plot?",('ROC Curve','Precision Recall Curve','Confusion Matrix'),key='1')
        
        if st.sidebar.button("Classify",key='class'):
            st.subheader("Decision Tree Result")
            Model = DecisionTreeClassifier(max_depth=max_depth_tree,criterion=criteria_tree)
            Model.fit(x_train,y_train)
            accuracy = Model.score(x_test,y_test)
            y_pred = Model.predict(x_test)
            st.write("Accuracy:",accuracy.round(2))
